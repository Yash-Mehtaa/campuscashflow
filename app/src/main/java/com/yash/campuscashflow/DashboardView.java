...
public class DashboardView extends VBox {
    ...
    private void onSaveCap() {
        try {
            long cents = Money.dollarsToCents(capField.getText());
            BudgetDao.upsertCap(YearMonth.now(), cents);
            capStatus.setText("Current cap: " + Money.format(cents));
            impactLbl.setText("");
            refreshAll();
            AppBus.notifyDataChanged(); // in case other tabs want to reflect cap change
        } catch (Exception ex) {
            showError("Invalid cap amount (e.g., 800 or 800.00)");
        }
    }
